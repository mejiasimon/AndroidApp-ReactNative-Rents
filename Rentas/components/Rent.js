import {
  ...
  import { rents } from "../services/information";
...
let filter1=vehiculo.filter(({plate,brand,state})=>{
  return plate==plateS
})
let filter2=vehiculo.filter(({plate,brand,state})=>{
  return state==1 && plate==plateS
})
let filter3=Usuarios.filter(({email,password,nombre})=>{
  return nombre==UserS
})
if(filter3[0]){
  if(filter1[0]){
